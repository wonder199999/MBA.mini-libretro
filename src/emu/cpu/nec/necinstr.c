...
OP(0x0f, i_pre_nec)
{
	...
	switch (FETCH())
	{
		...
		case 0x28 :
			...
			tmp <<= 4;
			tmp |= Breg(AL) & 0x0f;
			Breg(AL) = (Breg(AL) & 0xf0) | ((tmp>>8) & 0x0f);
			tmp &= 0xff;
...
static unsigned nec_i_popa_tmp;
OP( 0x61, i_popa  )
{
	POP(Wreg(IY));
	POP(Wreg(IX));
...
OP( 0x64, i_repnc  )
{
	...
	switch(next) {
		...
		default:	nec_instruction[next](nec_state); break;
	}
...
OP( 0x69, i_imul_d16 )
{
	...
	nec_state->CarryVal = nec_state->OverVal = (((INT32)dst) >> 15 != 0) && (((INT32)dst) >> 15 != -1);
	RegWord(ModRM)=(WORD)dst;
	nec_state->icount -= (ModRM >=0xc0 ) ? 38 : 47;
}
...
OP(0x6b, i_imul_d8)
{
	...
	nec_state->CarryVal = nec_state->OverVal = (((INT32)dst) >> 15 != 0) && (((INT32)dst) >> 15 != -1);
	RegWord(ModRM)=(WORD)dst;
	nec_state->icount -= (ModRM >=0xc0 ) ? 31 : 39;
}
...
OP( 0xd8, i_fpo    ) { nec_state->icount-=2; }
...
OP( 0xf4, i_hlt ) { nec_state->icount=0; }
...
OP( 0xf6, i_f6pre ) { UINT32 tmp; UINT32 uresult,uresult2; INT32 result,result2;
	...
	switch (ModRM & 0x38) {
		...
		case 0x28:
			result = (INT16)((INT8)Breg(AL))*(INT16)((INT8)tmp);
			Wreg(AW)=(WORD)result;
			nec_state->CarryVal = nec_state->OverVal = (Breg(AH)!=0);
			nec_state->icount-=(ModRM >=0xc0 ) ? 30 : 36;
			break; /* MUL */
...
OP( 0xf7, i_f7pre )
{
	...
	switch (ModRM & 0x38) {
		...
		case 0x20:
			uresult = Wreg(AW)*tmp;
			Wreg(AW)=uresult&0xffff;
			Wreg(DW)=((UINT32)uresult)>>16;
			nec_state->CarryVal=nec_state->OverVal=(Wreg(DW)!=0);
			nec_state->icount-=(ModRM >=0xc0 )?30:36;
			break; /* MULU */
		case 0x28:
			result = (INT32)((INT16)Wreg(AW))*(INT32)((INT16)tmp);
			Wreg(AW)=result&0xffff;
			Wreg(DW)=result>>16;
			nec_state->CarryVal=nec_state->OverVal=(Wreg(DW)!=0);
			nec_state->icount-=(ModRM >=0xc0) ? 30:36;
			break; /* MUL */
