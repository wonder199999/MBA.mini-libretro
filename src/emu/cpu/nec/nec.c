...
static CPU_RESET( nec )
{
	...
	nec_state->MF = 1;
	nec_state->ip = nec_state->TF = nec_state->IF = nec_state->DF = 0;
	nec_state->SignVal = nec_state->int_vector = nec_state->pending_irq = 0;
	nec_state->nmi_state = nec_state->irq_state = nec_state->poll_state = 0;

	nec_state->AuxVal = nec_state->OverVal = 0;
	nec_state->ZeroVal = nec_state->CarryVal = nec_state->ParityVal = 0;
	...
	{
		Mod_RM.RM.w[i] = (WREGS)(i & 0x07);
		Mod_RM.RM.b[i] = (BREGS)reg_name[i & 0x07];
	}
