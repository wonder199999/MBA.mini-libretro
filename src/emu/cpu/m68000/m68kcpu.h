...
#define DX	(REG_D[(m68k->ir >> 9) & 0x07])
#define DY	(REG_D[m68k->ir & 0x07])
/* Address Register Isolation */
#define AX	(REG_A[(m68k->ir >> 9) & 0x07])
#define AY	(REG_A[m68k->ir & 0x07])
...
#define XFLAG_AS_1(M)		(((M)->x_flag>>8)&0x01)
#define NFLAG_AS_1(M)		(((M)->n_flag>>7)&0x01)
#define VFLAG_AS_1(M)		(((M)->v_flag>>7)&0x01)
#define ZFLAG_AS_1(M)		(!(M)->not_z_flag)
#define CFLAG_AS_1(M)		(((M)->c_flag>>8)&0x01)
...
struct _m68ki_cpu_core
{
	...
	INT32		initial_cycles;
	INT32		remaining_cycles;	/* Number of clocks remaining */
	INT32		reset_cycles;
	UINT32		tracing;
...
INLINE unsigned int m68k_read_pcrelative_8(m68ki_cpu_core *m68k, unsigned int address)
{
	if (address >= m68k->encrypted_start && address < m68k->encrypted_end)
		return (((*m68k->memory.readimm16)(m68k->program, address & ~1) >> (8 * (1 - (address & 0x01)))) & 0xff);
...
INLINE void m68ki_set_sr_noint(m68ki_cpu_core *m68k, UINT32 value)
{
	...
	m68ki_set_sm_flag(m68k, (value >> 11) & 0x06);
}
...
INLINE void m68ki_set_sr_noint_nosp(m68ki_cpu_core *m68k, UINT32 value)
{
	...
	m68k->int_mask = value & 0x0700;
	m68ki_set_ccr(m68k, value);
	m68ki_set_sm_flag_nosp(m68k, (value >> 11) & 0x06);
}
