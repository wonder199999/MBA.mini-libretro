...
#define PIXEL_OP_COPY_OPAQUE(DEST, PRIORITY, SOURCE)	do { (DEST) = SOURCE; } while (0)	\
...
#define PIXEL_OP_COPY_TRANSPEN(DEST, PRIORITY, SOURCE)	do { UINT32 srcdata = (SOURCE); if (srcdata != transpen)(DEST) = SOURCE; } while (0)	\
...
#define PIXEL_OP_REMAP_OPAQUE(DEST, PRIORITY, SOURCE)	do { (DEST) = paldata[SOURCE]; } while (0)	\

#define PIXEL_OP_REMAP_OPAQUE_PRIORITY(DEST, PRIORITY, SOURCE)	do { if (((1 << ((PRIORITY) & 0x1f)) & pmask) == 0) (DEST) = paldata[SOURCE]; (PRIORITY) = 31; } while (0)	\
...
#define PIXEL_OP_REMAP_TRANSPEN(DEST, PRIORITY, SOURCE)		do { UINT32 srcdata = (SOURCE); if (srcdata != transpen) (DEST) = paldata[srcdata]; } while (0)		\
...
{												\
	...
	if (entry != DRAWMODE_NONE)								\
		(DEST) = (entry == DRAWMODE_SOURCE) ? paldata[srcdata] : shadowtable[DEST];	\
} while (0)											\
...
{														\
	...
	UINT32 entry = pentable[srcdata];									\
	if (entry != DRAWMODE_NONE)										\
		(DEST) = (entry == DRAWMODE_SOURCE) ? paldata[srcdata] : shadowtable[rgb_to_rgb15(DEST)];	\
} while (0)													\
...
#define PIXEL_OP_REMAP_TRANSPEN_ALPHA16(DEST, PRIORITY, SOURCE)	do { UINT32 srcdata = (SOURCE); if (srcdata != transpen) (DEST) = alpha_blend_r16((DEST), paldata[srcdata], alpha); } while (0)	\

#define PIXEL_OP_REMAP_TRANSPEN_ALPHA32(DEST, PRIORITY, SOURCE)	do { UINT32 srcdata = (SOURCE); if (srcdata != transpen) (DEST) = alpha_blend_r32((DEST), paldata[srcdata], alpha); } while (0)	\
