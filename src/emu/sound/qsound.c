...
struct _qsound_state
{
	...
	INT32			data;				/* register latch data */
	UINT32			sample_rom_length;
	INT32			pan_table[33];			/* Pan volume table */
...
static DEVICE_START( qsound )
{
	...
	/* Allocate stream */
	chip->stream = stream_create(device, 0, 2, device->clock() / QSOUND_CLOCKDIV, chip, qsound_update);
...
static STREAM_UPDATE( qsound_update )
{
	...
	for (int i = 0; i < QSOUND_CHANNELS; i++)
	{
		if (pC->key)
		{
