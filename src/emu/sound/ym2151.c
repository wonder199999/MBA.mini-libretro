...
#define FREQ_MASK	((1<<FREQ_SH)-1)	/* 65535 / 0xffff */
...
#define TL_TAB_LEN	(13*2*TL_RES_LEN)	/* 13*2*256=6656 */
#define ENV_QUIET	(TL_TAB_LEN>>3)		/* 6656/8=832 */

typedef struct _tables {
	INT32	tl_tab[TL_TAB_LEN];	/* 13*2*256 */
	UINT32	sin_tab[SIN_LEN];	/* 1 << 10  */
	UINT32	d1l_tab[16];
} tables;
...
typedef struct {
	...
	const UINT8 eg_inc[152];
	const UINT8 lfo_noise_waveform[256];
} eg_data;

static eg_data EG_RATE_TABLE = {
...
static void init_tables(void)
{
	...
	for (x = 0; x < TL_RES_LEN; x++)
	{
		...
		TAB.tl_tab[x * 2 + 0] = n;
		TAB.tl_tab[x * 2 + 1] = -(TAB.tl_tab[x * 2 + 0]);

		for (i = 1; i < 13; i++)
		{
			TAB.tl_tab[x * 2 + 0 + i * 2 * TL_RES_LEN] =  TAB.tl_tab[x * 2 + 0] >> i;
			TAB.tl_tab[x * 2 + 1 + i * 2 * TL_RES_LEN] = -(TAB.tl_tab[x * 2 + 0 + i * 2 * TL_RES_LEN]);
		}
	}
...
static void init_chip_tables(YM2151 *chip)
{
	...
	for (i = 0; i < 768; i++)
	{
		...
		chip->freq[3 * 768 + i] = ((int)(phaseinc * mult)) & 0xffffffc0;	/* adjust to X.10 fixed point */

		/* octave 0 and octave 1 */
		for (j = 0; j < 2; j++)
			chip->freq[768 + j * 768 + i] = (chip->freq[3 * 768 + i] >> (2 - j)) & 0xffffffc0;	/* adjust to X.10 fixed point */

		/* octave 3 to 7 */
		for (j = 3; j < 8; j++)
			chip->freq[768 + j * 768 + i] = chip->freq[3 * 768 + i] << (j - 2);
	}
	...
	const UINT8 dt1_tab[4*32] = { /* 4*32 DT1 values */
		/* DT1=0 */
...
#define KEY_ON(op, key_set) {								\
		...
		{									\
			...
			(op)->volume += (~(op)->volume * (SET->eg_inc[(op)->eg_sel_ar +	\
				((PSG->eg_cnt >> (op)->eg_sh_ar) & 0x07)])) >> 4;	\
...
static TIMER_CALLBACK( irqAon_callback )
{
	...
	if (oldstate == 0 && chip->irqhandler)
		(*chip->irqhandler)(chip->device, 1);
}
...
INLINE void refresh_EG(YM2151Operator * op)
{
	...
	if ((op->ar + v) < (32 + 62))
	{
		op->eg_sh_ar  = SET->eg_rate_shift[op->ar + v];
		op->eg_sel_ar = SET->eg_rate_select[op->ar + v];
	}
	else
	{
		op->eg_sh_ar  = 0;
	...
	op->eg_sh_d1r  = SET->eg_rate_shift[op->d1r + v];
	op->eg_sel_d1r = SET->eg_rate_select[op->d1r + v];
	op->eg_sh_d2r  = SET->eg_rate_shift[op->d2r + v];
	op->eg_sel_d2r = SET->eg_rate_select[op->d2r + v];
	op->eg_sh_rr   = SET->eg_rate_shift[op->rr + v];
	op->eg_sel_rr  = SET->eg_rate_select[op->rr + v];
...
void ym2151_write_reg(void *_chip, int r, int v)
{
	...
	switch (r & 0xe0)
	{
		case 0x00:
		{
			switch (r)
			{
				...
				case 0x08:
				{
					...
					envelope_KONKOFF(&chip->oper[(v & 0x07) * 4], v);
					break;
				}
				...
				{
					chip->timer_A_index = (chip->timer_A_index & 0x03) | (v << 2);
					break;
				}
				case 0x11:	/* timer A low */
				{
					chip->timer_A_index = (chip->timer_A_index & 0x3fc) | (v & 0x03);
					break;
				}
				case 0x12:	/* timer B */
				{
				...
				{
					chip->ct = v >> 6;
					chip->lfo_wsel = v & 0x03;
					if (chip->porthandler)
						(*chip->porthandler)(chip->device, 0, chip->ct);
					break;
				}
				default:
					break;
		...
		case 0x20:
		{
			op = &chip->oper[(r & 0x07) * 4];

			switch (r & 0x18)
			{
				case 0x00:	/* RL enable, Feedback, Connection */
				{
					op->fb_shift = ((v >> 3) & 0x07) ? ((v >> 3) & 0x07) + 6 : 0;
					chip->pan[(r & 0x07) * 2    ] = (v & 0x40) ? ~0 : 0;
					chip->pan[(r & 0x07) * 2 + 1] = (v & 0x80) ? ~0 : 0;
					chip->connect[r & 0x07] = v & 0x07;
					set_connect(op, r & 0x07, v & 0x07);
					break;
				}
				case 0x08:	/* Key Code */
				{
		...
		case 0x80:		/* KS, AR */
		{
			...
			if ((op->ar != oldar) || (op->ks != oldks))
			{
				...
				{
					op->eg_sh_ar  = SET->eg_rate_shift[op->ar + (op->kc >> op->ks)];
					op->eg_sel_ar = SET->eg_rate_select[op->ar + (op->kc >> op->ks)];
				}
				else
				{
					op->eg_sh_ar  = 0;
			...
			if (op->ks != oldks)
			{
				op->eg_sh_d1r = SET->eg_rate_shift[op->d1r + (op->kc >> op->ks)];
				op->eg_sel_d1r= SET->eg_rate_select[op->d1r + (op->kc >> op->ks)];
				op->eg_sh_d2r = SET->eg_rate_shift[op->d2r + (op->kc >> op->ks)];
				op->eg_sel_d2r= SET->eg_rate_select[op->d2r + (op->kc >> op->ks)];
				op->eg_sh_rr  = SET->eg_rate_shift[op->rr + (op->kc >> op->ks)];
				op->eg_sel_rr = SET->eg_rate_select[op->rr + (op->kc >> op->ks)];
			}
			break;
		}
...
{
	UINT32 p = (env << 3) + TAB.sin_tab[(((signed int)((OP->phase & ~FREQ_MASK) + (pm << 15))) >> FREQ_SH) & SIN_MASK];
...
INLINE signed int op_calc1(YM2151Operator * OP, unsigned int env, signed int pm)
{
	...
	UINT32 p = (env << 3) + TAB.sin_tab[(i >> FREQ_SH) & SIN_MASK];
...
INLINE void chan_calc(UINT32 chan)
{
	YM2151Operator *op = &PSG->oper[chan * 4];	/* M1 */
	UINT32 env, AM = 0;
...
{
	YM2151Operator *op = &PSG->oper[7 * 4];	/* M1 */
	UINT32 env, AM = 0;
		...
		do
		{
			switch (op->state)
			{
				case EG_ATT:	/* attack phase */
				{
					...
					{
						op->volume += (~op->volume * (SET->eg_inc[op->eg_sel_ar + ((PSG->eg_cnt >> op->eg_sh_ar) & 0x07)])) >> 4;
				...
				case EG_DEC:	/* decay phase */
				{
					...
					{
						op->volume += SET->eg_inc[op->eg_sel_d1r + ((PSG->eg_cnt >> op->eg_sh_d1r) & 0x07)];
				...
				case EG_SUS:	/* sustain phase */
				{
					...
					{
						op->volume += SET->eg_inc[op->eg_sel_d2r + ((PSG->eg_cnt >> op->eg_sh_d2r) & 0x07)];
				...
				case EG_REL:	/* release phase */
				{
					...
					{
						op->volume += SET->eg_inc[op->eg_sel_rr + ((PSG->eg_cnt>>op->eg_sh_rr) & 0x07)];
...
INLINE void advance(void)
{
	...
	while (i)
	{
		UINT32 j = ((PSG->noise_rng ^ (PSG->noise_rng >> 3)) & 0x01) ^ 1;
...
void ym2151_update_one(void *chip, SAMP **buffers, int length)
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		chanout[0] = chanout[1] = 0;
		chanout[2] = chanout[3] = 0;
		chanout[4] = chanout[5] = 0;
		chanout[6] = chanout[7] = 0;
