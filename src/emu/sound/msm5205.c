...
{
	msm5205_state *info = (msm5205_state *)ptr;
	int val, new_signal;

	/* callback user handler and latch next data */
	if (info->intf->vclk_callback)
		(*info->intf->vclk_callback)(info->device);
	...
	else
	{
		...
		new_signal = info->signal + info->diff_lookup[info->step * 16 + (val & 0x0f)];
		if (new_signal > 2047)
			new_signal = 2047;
		else if (new_signal < -2048)
			new_signal = -2048;

		info->step += index_shift[val & 0x07];
		if (info->step > 48)
			info->step = 48;
		else if (info->step < 0)
			info->step = 0;
	...
	if (info->signal != new_signal)
	{
		stream_update(info->stream);
		info->signal = new_signal;
	}
}
...
void msm5205_vclk_w (running_device *device, int vclk)
{
	...
	if (voice->prescaler == 0)
	{
		if (voice->vclk != vclk)
		{
			voice->vclk = vclk;
			if (!vclk)
				MSM5205_vclk_callback(voice->device->machine, voice, 0);
		}
	}
}
...
{
	msm5205_state *voice = get_safe_token(device);
	voice->data = (voice->bitwidth == 4) ? data & 0x0f : (data & 0x07) << 1;
}
...
{
	int prescaler = 0;
	int bitwidth = (select & 0x04) ? 4 : 3;

	switch (select & 0x03)
	{
		case 0: prescaler = 96; break;
		case 1: prescaler = 48; break;
