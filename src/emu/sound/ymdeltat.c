...
static YTB ym_deltat_decode_table = 
{
...
*/
		...
		case 0x01:	/* L,R,-,-,SAMPLE,DA/AD,RAMTYPE,ROM */
		{
			...
			if ((DELTAT->control2 & 0x03) != (v & 0x03))
			{
				/*0-DRAM x1, 1-ROM, 2-DRAM x8, 3-ROM (3 is bad setting - not allowed by the manual) */
				if (DELTAT->DRAMportshift != dram_rightshift[v & 0x03])
				{
					DELTAT->DRAMportshift = dram_rightshift[v & 0x03];
		...
		case 0x03:	/* Start Address H */
			DELTAT->start = (DELTAT->reg[0x03] * 0x0100 | DELTAT->reg[0x02]) << (DELTAT->portshift - DELTAT->DRAMportshift);
		break;
...
*/
		...
		case 0x0a:	/* DELTA-N H */
			DELTAT->delta = (DELTAT->reg[0x0a] * 0x0100 | DELTAT->reg[0x09]);
			DELTAT->step  = (UINT32)((double)(DELTAT->delta) * (DELTAT->freqbase));
		break;
...
*/
		...
		case 0x0d:	/* Limit Address H */
			DELTAT->limit = (DELTAT->reg[0x0d] * 0x0100 | DELTAT->reg[0x0c]) << (DELTAT->portshift - DELTAT->DRAMportshift);
		break;
	}
}
...
void YM_DELTAT_ADPCM_Reset(YM_DELTAT *DELTAT, int pan, int emulation_mode)
{
	...
	DELTAT->DRAMportshift = dram_rightshift[DELTAT->control2 & 0x03];
...
{
	YTB *table = &ym_deltat_decode_table;
	UINT32 step;
	int data;
	...
	if (DELTAT->now_step >= (1 << YM_DELTAT_SHIFT))
	{
		...
		do
		{
			...
			DELTAT->acc += table->ym_deltat_decode_tableB1[data] * DELTAT->adpcmd / 8;
			YM_DELTAT_Limit(DELTAT->acc, YM_DELTAT_DECODE_MAX, YM_DELTAT_DECODE_MIN);

			/* delta to next delta */
			DELTAT->adpcmd = (DELTAT->adpcmd * table->ym_deltat_decode_tableB2[data]) / 64;
