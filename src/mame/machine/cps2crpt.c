...
static void cps2_decrypt(running_machine *machine, const UINT32 *master_key, UINT32 upper_limit)
{
	...
	for (i = 0; i < 0x10000; ++i)
	{
		...
		// decrypt the opcodes
		for (a = i; a < tmp1 && a < tmp2; a += 0x10000)
		{
...
static const struct game_keys keys_table[] =
{
	...
	{ "ssf2tbh",   	{ 0xbcdef012, 0x3456789a }, 0x400000 },
	...
	{ "ssf2th",  	{ 0x940e2d02, 0x75211432 }, 0x400000 },
	...
	{ "ringdesta", 	{ 0x19940727, 0x17452103 }, 0x180000 },
	{ "ringdesth", 	{ 0x19940727, 0x17473803 }, 0x180000 },
	...
	{ "vhuntjr1s", 	{ 0x1135b2c3, 0xa4e9d7f2 }, 0x180000 }, 
	...
	{ "mmancp2ur1", { 0x054893fa, 0x94642525 }, 0x100000 },	
	...
	{ "ddsomar1",  	{ 0x8719abcd, 0xef028345 }, 0x100000 }, 
	...
	{ "csclubjy",  	{ 0x4a2d0be5, 0x56c013c0 }, 0x200000 },	
	...
	{ "mvscjsing",	{ 0xafc16138, 0x97123eb0 }, 0x100000 },
...
DRIVER_INIT( cps2crpt )
{
	...
	while (k->name)
	{
		...
		{
			cps2_decrypt(machine, k->keys, k->upper_limit ? k->upper_limit : 0x400000);	/* we have a proper key so use it to decrypt */
			break;
		}
		++k;
	}
