...
static READ16_HANDLER( sf2dongb_prot_r )
{
	UINT16 ret = (offset == 0x000000 || offset == 0x03b820) ? 0x0210 : 0x0000;
	return ret;
}

static READ16_HANDLER( sf2rb_prot_lo_r )
{
	cps_state *state = space->machine->driver_data<cps_state>();

	if (state->bootleg_sf2rb2)
		return 0x0000;
	else
		return 0x0002;
}

static READ16_HANDLER( sf2rb_prot_hi_r )
{
	return 0x0040;
}

static WRITE16_HANDLER( sf2hfjb_layer_w )
{
	cps_state *state = space->machine->driver_data<cps_state>();
	state->cps_a_regs[0x20 / 2] = data;
}
...
static WRITE16_HANDLER( dinohack_sound_command_w )
{
	...
	state->qsound_sharedram1[1] = data;
}
...
{
	int in1 = input_port_read(space->machine, "IN1");
	return (in1 << 8) | in1;
}

READ16_HANDLER( cps1_in2_r )
{
	int in2 = input_port_read(space->machine, "IN2");
	return (in2 << 8) | in2;
}

READ16_HANDLER( cps1_in3_r )
{
	int in3 = input_port_read(space->machine, "IN3");
	return (in3 << 8) | in3;
}
...
static ADDRESS_MAP_START( main_map, ADDRESS_SPACE_PROGRAM, 16 )
	...
	/* CPS-B custom is mapped by a PAL on B-board. SF2 revision "E" USA 910228 has it a a different address, see DRIVER_INIT */
...
static INPUT_PORTS_START( stridrua )
	...
	/* In 'striderj', bit 3 is stored at 0xff8e77 ($e77,A5) via code at 0x000a2a,
	   but this address is never checked again. in 'strider' and 'stridrjr', this code even doesn't exist ! */
...
static INPUT_PORTS_START( mercs )
	...
	PORT_START("DSWA")
	CPS1_COINAGE_2( "SW(A)" )
	PORT_DIPUNUSED_DIPLOC( 0x08, 0x08, "SW(A):4" )		/* These three switches are not documented in the manual */
	...
	PORT_DIPUNUSED_DIPLOC( 0x20, 0x20, "SW(B):6" )		/* These three switches are not documented in the manual */
	PORT_DIPUNUSED_DIPLOC( 0x40, 0x40, "SW(B):7" )
	PORT_DIPUNUSED_DIPLOC( 0x80, 0x80, "SW(B):8" )

	PORT_START("DSWC")
	PORT_DIPUNUSED_DIPLOC( 0x01, 0x01, "SW(C):1" )		/* These three switches are not documented in the manual */
	PORT_DIPUNUSED_DIPLOC( 0x02, 0x02, "SW(C):2" )
	PORT_DIPUNUSED_DIPLOC( 0x04, 0x04, "SW(C):3" )
	PORT_DIPNAME( 0x08, 0x08, "Freeze" )			PORT_DIPLOCATION("SW(C):4")
	PORT_DIPSETTING(    0x08, DEF_STR( Off ) )
...
static INPUT_PORTS_START( sf2bhh )
	PORT_INCLUDE( sf2 )
...
static MACHINE_DRIVER_START( sf2m3 )
	/* basic machine hardware */
	MDRV_IMPORT_FROM(cps1_12MHz)
...
ROM_START( ffightu2 )
	...
	ROM_REGION( 0x18000, "audiocpu", 0 )		/* 64k for the audio CPU (+banks) */
	ROM_LOAD( "s.13c",  0x00000, 0x08000, CRC(b8367eb5) SHA1(ec3db29fdd6200e9a8f4f8073a7e34aef731354f) )
	ROM_CONTINUE(	    0x10000, 0x08000 )
...
ROM_START( knightsh )
	...
	ROM_REGION( 0x400000, "gfx", 0 )
	ROMX_LOAD( "kr_gfx5.rom",  0x000000, 0x80000, CRC(9e36c1a4) SHA1(772daae74e119371dfb76fde9775bda78a8ba125) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "1" socket
	ROMX_LOAD( "kr_gfx7.rom",  0x000002, 0x80000, CRC(c5832cae) SHA1(a188cf401cd3a2909b377d3059f14d22ec3b0643) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "3" socket
	ROMX_LOAD( "kr_gfx1.rom",  0x000004, 0x80000, CRC(f095be2d) SHA1(0427d1574062f277a9d04440019d5638b05de561) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "2" socket
	ROMX_LOAD( "kr_gfx3.rom",  0x000006, 0x80000, CRC(179dfd96) SHA1(b1844e69da7ab13474da569978d5b47deb8eb2be) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "4" socket
	ROMX_LOAD( "kr_gfx6.rom",  0x200000, 0x80000, CRC(1f4298d2) SHA1(4b162a7f649b0bcd676f8ca0c5eee9a1250d6452) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "5" socket
	ROMX_LOAD( "kr_gfx8.rom",  0x200002, 0x80000, CRC(37fa8751) SHA1(b88b39d1f08621f15a5620095aef998346fa9891) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "7" socket
	ROMX_LOAD( "kr_gfx2.rom",  0x200004, 0x80000, CRC(0200bc3d) SHA1(c900b1be2b4e49b951e5c1e3fd1e19d21b82986e) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "6" socket
	ROMX_LOAD( "kr_gfx4.rom",  0x200006, 0x80000, CRC(0bb2b4e7) SHA1(983b800925d58e4aeb4e5105f93ed5faf66d009c) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "8" socket
...
ROM_START( knightsb2 )
	...
	ROM_REGION( 0x400000, "gfx", 0 )
	ROMX_LOAD( "kr-5m.rom",  0x000000, 0x80000, CRC(9e36c1a4) SHA1(772daae74e119371dfb76fde9775bda78a8ba125) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "1" socket
	ROMX_LOAD( "kr-7m.rom",  0x000002, 0x80000, CRC(c5832cae) SHA1(a188cf401cd3a2909b377d3059f14d22ec3b0643) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "3" socket
	ROMX_LOAD( "kr-1m.rom",  0x000004, 0x80000, CRC(f095be2d) SHA1(0427d1574062f277a9d04440019d5638b05de561) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "2" socket
	ROMX_LOAD( "kr-3m.rom",  0x000006, 0x80000, CRC(179dfd96) SHA1(b1844e69da7ab13474da569978d5b47deb8eb2be) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "4" socket
	ROMX_LOAD( "kr-6m.rom",  0x200000, 0x80000, CRC(1f4298d2) SHA1(4b162a7f649b0bcd676f8ca0c5eee9a1250d6452) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "5" socket
	ROMX_LOAD( "kr-8m.rom",  0x200002, 0x80000, CRC(37fa8751) SHA1(b88b39d1f08621f15a5620095aef998346fa9891) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "7" socket
	ROMX_LOAD( "kr-2m.rom",  0x200004, 0x80000, CRC(0200bc3d) SHA1(c900b1be2b4e49b951e5c1e3fd1e19d21b82986e) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "6" socket
	ROMX_LOAD( "kr-4m.rom",  0x200006, 0x80000, CRC(0bb2b4e7) SHA1(983b800925d58e4aeb4e5105f93ed5faf66d009c) , ROM_GROUPWORD | ROM_SKIP(6) ) // in "8" socket
...
ROM_START( kod )
	...
	ROM_REGION( 0x0200, "cboardplds", 0 )
	ROM_LOAD( "iob1.11d",     0x0000, 0x0117, NO_DUMP )
	ROM_LOAD( "ioc1.ic7",     0x0000, 0x0117, CRC(0d182081) SHA1(475b3d417785da4bc512cce2b274bb00d4cc6792) )
	ROM_LOAD( "c632.ic1",     0x0000, 0x0117, CRC(0fbd9270) SHA1(d7e737b20c44d41e29ca94be56114b31934dde81) )
ROM_END
...
ROM_START( kodda )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )      /* 68000 code */
	ROM_LOAD16_WORD_SWAP( "8f",  0x00000, 0x80000, CRC(0a6ab826) SHA1(5156737304d3084e03dfb9e11c2a023f4b4da5ad) )
	ROM_LOAD16_WORD_SWAP( "7f",  0x80000, 0x80000, CRC(9bd7ad4b) SHA1(7bece5d408fd13116bd5518014b632ecc9a2feaa) )

	ROM_REGION( 0x400000, "gfx", 0 )
	ROMX_LOAD( "kd-5m.4a",	0x000000, 0x80000, CRC(e45b8701) SHA1(604e39e455e81695ee4f899f102d0bcd789cedd0) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "2" socket
	ROMX_LOAD( "kd-7m.6a",	0x000002, 0x80000, CRC(a7750322) SHA1(3c583496a53cd64edf377db35f7f40f02b59b7e7) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "4" socket
	ROMX_LOAD( "kd-1m.3a",	0x000004, 0x80000, CRC(5f74bf78) SHA1(b7c43eea9bf77a0fb571dcd53f8be719e6655fd9) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "1" socket
	ROMX_LOAD( "kdda-7m.6a",0x000006, 0x80000, CRC(0e2cb76f) SHA1(2892caed6bf98378ce01648408fda162a8121ec0) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "3" socket
	ROMX_LOAD( "kd-6m.4c",	0x200000, 0x80000, CRC(113358f3) SHA1(9d98eafa74a046f65bf3847fe1d88ea1b0c82b0c) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "11" socket
	ROMX_LOAD( "kd-8m.6c",	0x200002, 0x80000, CRC(38853c44) SHA1(a6e552fb0138a76a7878b90d202904e2b44ae7ec) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "13" socket
	ROMX_LOAD( "kd-2m.3c",	0x200004, 0x80000, CRC(9ef36604) SHA1(b42ca0a910b65e1e7bb6e7d734e853ce67e821bf) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "10" socket
	ROMX_LOAD( "kd-4m.5c",	0x200006, 0x80000, CRC(402b9b4f) SHA1(4c11976976eadf1ad293b31b0a4d047d05032b06) , ROM_GROUPWORD | ROM_SKIP(6) )  // in "12" socket
...
ROM_START( pang3b2 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )		/* 68000 code */
	ROM_LOAD16_WORD_SWAP( "pa3_17bl.11l",  0x00000, 0x80000, CRC(3b5d99de) SHA1(d3cb222f30d3a45de6114e4377f9934f33bf6bcf) )
	ROM_LOAD16_WORD_SWAP( "pa3_16bl.10l",  0x80000, 0x80000, CRC(1be9a483) SHA1(6cff1dd15ca163237bc82fb4a3e1d469d35e7be8) )
...
ROM_START( pang3b3 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )		/* 68000 code */
	ROM_LOAD16_WORD_SWAP( "17.11l",  0x00000, 0x80000, CRC(f62425e9) SHA1(658b60efc0467a173a1dd9bd5ac88f49f02f5de2) )
	ROM_LOAD16_WORD_SWAP( "16.10l",  0x80000, 0x80000, CRC(7169ea67) SHA1(1076afaff7b05c9009798a0cea13e71fa27f2b7e) )

	ROM_REGION( 0x400000, "gfx", 0 )
	ROMX_LOAD( "1.2c", 0x000000, 0x100000, CRC(068a152c) SHA1(fa491874068924c39bcc7de93dfda3b27f5d9613) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(	   0x000004, 0x100000 )
	ROMX_LOAD( "7.2f", 0x000002, 0x100000, CRC(3a4a619d) SHA1(cfe68e24632b53fb6cd6d03b2166d6b5ba28b778) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(	   0x000006, 0x100000 )

	ROM_REGION( 0x28000, "audiocpu", 0 )		/* 64k for the audio CPU (+banks) */
	ROM_LOAD( "11.11f",  0x00000, 0x08000, CRC(cb1423a2) SHA1(3191bf5d340168647881738cb2aed09b1d86146e) )
	ROM_CONTINUE(	     0x10000, 0x18000 )

	ROM_REGION( 0x40000, "oki", 0 )			/* Samples */
	ROM_LOAD( "5.10d",  0x00000, 0x20000, CRC(73a10d5d) SHA1(999465e4fbc35a34746d2db61ad49f61403d5af7) )
...
ROM_START( pang3r1a )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )      /* 68000 code */
	ROM_LOAD16_WORD_SWAP( "d.t.11l", 0x00000, 0x80000, CRC(d7041d32) SHA1(b021f3defe7fc58030ba907125c713f987724187) )
	ROM_LOAD16_WORD_SWAP( "d.t.10l", 0x80000, 0x80000, CRC(1be9a483) SHA1(6cff1dd15ca163237bc82fb4a3e1d469d35e7be8) )

	ROM_REGION( 0x400000, "gfx", 0 )
	ROMX_LOAD( "mx27c4100.2c", 0x000000, 0x80000, CRC(22c934c4) SHA1(93a3c3cfdbfd1321b785f08d1da30d9b1445e14b) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.3c", 0x200000, 0x80000, CRC(07c85e9b) SHA1(5b9bc1f5708d03c2458a1dbb781084c2af8f91ee) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.4c", 0x000004, 0x80000, CRC(ac119a46) SHA1(e8a07349b6106f712a7543ab52e9d5ced756fdbd) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.5c", 0x200004, 0x80000, CRC(4ad13297) SHA1(59565f9819e1cdd405103d5e3621c747116f653c) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.2f", 0x000002, 0x80000, CRC(51031180) SHA1(a38a77da2ca452843a3b0cb02baaaf7df2e3d9a4) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.3f", 0x200002, 0x80000, CRC(e9cd657a) SHA1(98738158992e813c6a9c3806fed0a7e02b859ad3) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.4f", 0x000006, 0x80000, CRC(2e1d35f2) SHA1(1b4a9cf9ed91fed532d44582c598982dae06253c) , ROM_GROUPWORD | ROM_SKIP(6) )
	ROMX_LOAD( "mx27c4100.5f", 0x200006, 0x80000, CRC(026d0cd2) SHA1(44f7718851e0b1f43682afafcbd844e6b1ce3430) , ROM_GROUPWORD | ROM_SKIP(6) )

	ROM_REGION( 0x18000, "audiocpu", 0 ) /* 64k for the audio CPU (+banks) */
	ROM_LOAD( "d.t.11f",  0x00000, 0x08000, CRC(cb1423a2) SHA1(3191bf5d340168647881738cb2aed09b1d86146e) )
	ROM_IGNORE( 0x18000 )

	ROM_REGION( 0x40000, "oki", 0 )	/* Samples */
	ROM_LOAD( "d.t.10d",  0x00000, 0x20000, CRC(73a10d5d) SHA1(999465e4fbc35a34746d2db61ad49f61403d5af7) )
	ROM_LOAD( "d.t.11d",  0x20000, 0x20000, CRC(affa4f82) SHA1(27b9292bbc121cf585f53297a79fe8f0d0a729ae) )
...
ROM_START( wofchdx )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
	ROM_LOAD16_WORD_SWAP( "tk2(ch)dx_23.8f",  0x000000, 0x080000, CRC(29389b7d) SHA1(ccc4a7c4cb43abe9e8f9fd0d1a813f722473784f) )
	ROM_LOAD16_WORD_SWAP( "tk2(ch)dx_22.7f",  0x080000, 0x080000, CRC(a959df0c) SHA1(ec6d9f1f9363dee08ea267fab28d313e0b46d3da) )
...
ROM_START( sf2hfjb )	// This set is also known as sf2ce13
	ROM_REGION( CODE_SIZE, "maincpu", 0 )      /* 68000 code */
	...
	ROM_REGION( 0x600000, "gfx", 0 )
	ROMX_LOAD( "s92_01.bin",    0x000000, 0x80000, CRC(03b0d852) SHA1(f370f25c96ad2b94f8c53d6b7139100285a25bef) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-1m.3a
	ROMX_LOAD( "s92_02.bin",    0x000002, 0x80000, CRC(840289ec) SHA1(2fb42a242f60ba7e74009b5a90eb26e035ba1e82) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-3m.5a
	ROMX_LOAD( "s92_03.bin",    0x000004, 0x80000, CRC(cdb5f027) SHA1(4c7d944fef200fdfcaf57758b901b5511188ed2e) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-2m.4a
	ROMX_LOAD( "s92_04.bin",    0x000006, 0x80000, CRC(e2799472) SHA1(27d3796429338d82a8de246a0ea06dd487a87768) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-4m.6a
	ROMX_LOAD( "s92_05.bin",    0x200000, 0x80000, CRC(ba8a2761) SHA1(4b696d66c51611e43522bed752654314e76d33b6) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-5m.7a
	ROMX_LOAD( "s92_06.bin",    0x200002, 0x80000, CRC(e584bfb5) SHA1(ebdf1f5e2638eed3a65dda82b1ed9151a355f4c9) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-7m.9a
	ROMX_LOAD( "s92_07.bin",    0x200004, 0x80000, CRC(21e3f87d) SHA1(4a4961bb68c3a1ce15f9d393d9c03ecb2466cc29) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-6m.8a
	ROMX_LOAD( "s92_08.bin",    0x200006, 0x80000, CRC(befc47df) SHA1(520390420da3a0271ba90b0a933e65143265e5cf) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-8m.10a
...
ROM_START( sf2hfjb2 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )      /* 68000 code */
	ROM_LOAD16_BYTE( "sf12-1.040",   0x000000, 0x80000, CRC(ec6f5cb3) SHA1(24b3fc88a08a721985b053120997090bc394bc8e) )
	ROM_LOAD16_BYTE( "sf12-3.040",   0x000001, 0x80000, CRC(0e9ac52b) SHA1(2c5faf7d8b598f2110f389aefa1a02e2281382ee) )
	ROM_LOAD16_BYTE( "sf12-2.010",   0x100000, 0x20000, CRC(d1707134) SHA1(5bfdf7bc57bdb85183647ebb175346070dd102ee) )
	ROM_LOAD16_BYTE( "sf12-4.010",   0x100001, 0x20000, CRC(cd1d5666) SHA1(8befd2c324eb29bdad6fc8fb2554cdfaf9808f9b) )
...
ROM_START( sf2stt )
	...
	ROM_LOAD16_BYTE( "09.bin",   0x00001, 0x40000, CRC(59ccd474) SHA1(7bb28c28ee722435fdbb18eb73e52bd65b419103) )
	*/
	ROM_LOAD16_BYTE( "prg part 1.stt", 0x00000, 0x40000, CRC(a258b4d5) SHA1(3433b6493794c98bb35c1b27cc65bb5f13d52e9b) )
	ROM_LOAD16_BYTE( "prg part 2.stt", 0x00001, 0x40000, CRC(59ccd474) SHA1(7bb28c28ee722435fdbb18eb73e52bd65b419103) )
...
ROM_START( sf2amf )
	...
	// wasn't in original dump (set seems to be same as sf2amf7 but with different graphics roms)
...
ROM_START( sf2amf3 )		// this set was previously known as sf2m7
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
...
ROM_START( sf2amf4 )		// this set was previously known as sf2m10 ?
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
...
ROM_START( sf2amf5 )
	...
	ROM_LOAD( "sf2th-8.bin", 0x00000, 0x10000, CRC(13ea1c44) SHA1(5b05fe4c3920e33d94fac5f59e09ff14b3e427fe) )
ROM_END
...
ROM_START( sf2amf6 )
	...
	ROM_LOAD( "g01.bin", 0x00000, 0x10000, CRC(13ea1c44) SHA1(5b05fe4c3920e33d94fac5f59e09ff14b3e427fe) )

	ROM_REGION( 0x28000, "user2", 0 )
	ROM_LOAD( "palce16v8h-7.u35", 0x00000, 0x10000, CRC(13ea1c44) SHA1(5b05fe4c3920e33d94fac5f59e09ff14b3e427fe) )
	ROM_LOAD( "plhs16l8an-8.u1",  0x10000, 0x01946, CRC(50852ce7) SHA1(f34321acf8fd746da25e4b7ec30eb6ca08c44dbf) )
	ROM_LOAD( "palce16v8h-9.u34", 0x18000, 0x03254, CRC(feb26c41) SHA1(095d95057b00a67a04da0e4998579efa812602a9) )
	ROM_LOAD( "plhs16l8an-x.u19", 0x20000, 0x01946, CRC(63d7d7ba) SHA1(c11558811bb077a7dc16bc147718de048e4b438e) )
ROM_END

ROM_START( sf2amf7 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
	ROM_LOAD16_BYTE( "2.u222", 0x000000, 0x80000, CRC(3a7a78f8) SHA1(6d3502459d2fced88920689191358594c04e1b82) )
	ROM_LOAD16_BYTE( "1.u196", 0x000001, 0x80000, CRC(6a3cb7ac) SHA1(ca4a52e65a50286c5fb7a1032b5d4a82d23a20b6) )
	ROM_LOAD16_BYTE( "4.u221", 0x100000, 0x40000, CRC(b0bb6242) SHA1(453655f8767c20cbbd52bc847497408cb239da1c) )
	ROM_LOAD16_BYTE( "3.u195", 0x100001, 0x40000, CRC(f516e578) SHA1(82fe84bd24ebc46be9d733506c6c1528035bd62d) )

	ROM_REGION( 0x600000, "gfx", 0 )
	ROMX_LOAD( "y.c.e.c m.k.r-001.u70", 0x000000, 0x80000, CRC(a258de13) SHA1(2e477948c4c8a2fb7cfdc4a739766bc4a4e01c49), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x000004, 0x80000 )
	ROMX_LOAD( "y.c.e.c m.k.r-003.u68", 0x000002, 0x80000, CRC(c781bf87) SHA1(034baa9807c2ce8dc800200963a38cd9262b21fb), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x000006, 0x80000 )
	ROMX_LOAD( "y.c.e.c m.k.r-002.u69", 0x200000, 0x80000, CRC(5726cab8) SHA1(0b2243a9a7184d53d42ddab7a8c51b63001c2f56), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x200004, 0x80000 )
	ROMX_LOAD( "y.c.e.c d.w.c-011.u64", 0x200002, 0x80000, CRC(bc90c12f) SHA1(ecdb776239b22bd56b7c3a87c9e561f650a4dfea), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x200006, 0x80000 )
	ROMX_LOAD( "y.c.e.c d.w.c-012.u19", 0x400000, 0x80000, CRC(187667cc) SHA1(fae65bf23f49a32903fda8080659ccf8d42b911f), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x400004, 0x80000 )
	ROMX_LOAD( "y.c.e.c d.w.c-013.u18", 0x400002, 0x80000, CRC(5b585071) SHA1(ad3371b1ba0441c67d9fcbb23b09464710e4e28a), ROM_GROUPWORD | ROM_SKIP(6) )
	ROM_CONTINUE(			    0x400006, 0x80000 )
	/* extra gfx layer roms loaded over the former ones to remove the capcom copyright logo */
	ROMX_LOAD( "grp1.u31",	0x400004, 0x10000, CRC(6de44671) SHA1(dc6abba639e0c27033e391c7438d88dc89a93351), ROM_SKIP(7) )
	ROM_CONTINUE(		0x400000, 0x10000 )
...
ROM_START( sf2amf8 )		// this set was previously known as sf2m5
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
...
ROM_START( sf2cejabl )
	...
	ROMX_LOAD( "s92_07.bin",  0x200004, 0x80000, CRC(21e3f87d) SHA1(4a4961bb68c3a1ce15f9d393d9c03ecb2466cc29) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-6m.8a
	ROMX_LOAD( "s92_08.bin",  0x200006, 0x80000, CRC(befc47df) SHA1(520390420da3a0271ba90b0a933e65143265e5cf) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-8m.10a
	ROMX_LOAD( "s92_10.bin",  0x400000, 0x80000, CRC(960687d5) SHA1(2868c31121b1c7564e9767b9a19cdbf655c7ed1d) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-10m.3c
	ROMX_LOAD( "s92_11.bin",  0x400002, 0x80000, CRC(978ecd18) SHA1(648a59706b93c84b4206a968ecbdc3e834c476f6) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-12m.5c
	ROMX_LOAD( "s92_12.bin",  0x400004, 0x80000, CRC(d6ec9a0a) SHA1(ed6143f8737013b6ef1684e37c05e037e7a80dae) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-11m.4c
	ROMX_LOAD( "s92_13.bin",  0x400006, 0x80000, CRC(ed2c67f6) SHA1(0083c0ffaf6fe7659ff0cf822be4346cd6e61329) , ROM_GROUPWORD | ROM_SKIP(6) )	// == s92-13m.6c
...
ROM_START( sf2cejab2 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
	ROM_LOAD16_BYTE( "u7-1.040",	     0x000000, 0x80000, CRC(866a9b31) SHA1(92f89084293b69a9d5cd8d282ffaaca0739f1b03) )
	ROM_LOAD16_BYTE( "u7-3.040",	     0x000001, 0x80000, CRC(49b15fcd) SHA1(4edc998e559568730e0fcc122162beefac541337) )
	ROM_LOAD16_WORD_SWAP( "s92_21a.bin", 0x100000, 0x80000, CRC(925a7877) SHA1(1960dca35f0ca6f2b399a9fccfbc0132ac6425d1) )
...
ROM_START( sf2red2 )
	...
	ROM_LOAD16_BYTE( "sf3d5-4010", 0x100001, 0x20000, CRC(c95e4443) SHA1(28417dee9ccdfa65b0f4a92aa29b90279fe8cd85) )
...
ROM_START( sf2rb4 )		// this set was previously known as sf2m4
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
...
ROM_START( sf2rb6 )
	ROM_REGION( CODE_SIZE, "maincpu", 0 )	/* 68000 code */
	ROM_LOAD16_WORD_SWAP( "cebl_23", 0x000000, 0x80000, CRC(1439fcad) SHA1(7fc3557c24f5cfb4ceab03c3354d9b773289b7bc) )
	ROM_LOAD16_WORD_SWAP( "cebl_22", 0x080000, 0x80000, CRC(27e80cb1) SHA1(d32eba8c7c887d6149020ab103ecda13448e6608) )
	ROM_LOAD16_WORD_SWAP( "cebl_21", 0x100000, 0x80000, CRC(f21e3046) SHA1(ac630aafb98ab579c1855e0ee30251ba3922892e) )
...
static DRIVER_INIT( dinotpic )
{
	const int patch_table[69][2] = {
		...
		{ 0x0486, 0x80 }, { 0x0487, 0x00 }, { 0x0488, 0x04 }, { 0x0489, 0x01 }, { 0x048A, 0xFC }, { 0x048B, 0x33 },
		{ 0x048C, 0x00 }, { 0x048D, 0x91 }, { 0x048E, 0x80 }, { 0x048F, 0x00 }, { 0x0490, 0x06 }, { 0x0491, 0x01 },
		{ 0x06cc, 0x80 }, { 0x06cd, 0x00 }, { 0x06ce, 0x0C }, { 0x06cf, 0x01 }, { 0x06de, 0x80 }, { 0x06df, 0x00 },
...
static DRIVER_INIT( sf2hfjb )
{
	const int fix_gfx_table[][2] = {
		{ 0x10df6, 0x0083 }, { 0x10e7c, 0x828e }, { 0x10e7d, 0x829e }, { 0x10e7e, 0x82ae }, { 0x10e83, 0x827f },
		{ 0x10e84, 0x828f }, { 0x10e85, 0x829f }, { 0x10e86, 0x82af }, { 0x10e8b, 0x827e }, { 0x10e8c, 0x82a0 },
...
static DRIVER_INIT( sf2rb )
{
	...
	memory_install_read16_handler(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), 0x281200, 0x281201, 0, 0, sf2rb_prot_hi_r);

	DRIVER_INIT_CALL(cps1);
}

static DRIVER_INIT( sf2dongb )
{
	/* There is a hacked up Altera EP910PC-30 DIP in the 5f socket instead of a 4th EPROM */
	memory_install_read16_handler(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), 0x180000, 0x1fffff, 0, 0, sf2dongb_prot_r);
	DRIVER_INIT_CALL(cps1);
}

static DRIVER_INIT( sf2rb6 )
{
	UINT8 *src = (UINT8 *)memory_region(machine, "maincpu");
	UINT8 *dst = auto_alloc_array(machine, UINT8, 0x180000);

	if (dst != NULL)
	{
		memcpy(dst, src, 0x180000);

		memcpy(src + 0x080000, dst + 0x000000, 0x40000);
		memcpy(src + 0x140000, dst + 0x040000, 0x40000);
		memcpy(src + 0x100000, dst + 0x080000, 0x40000);
		memcpy(src + 0x040000, dst + 0x0c0000, 0x40000);
		memcpy(src + 0x000000, dst + 0x100000, 0x40000);
		memcpy(src + 0x0c0000, dst + 0x140000, 0x40000);
...
//
GAME( 1988,	forgottn,	0,		cps1_10MHz,	forgottn,	forgottn,	ROT0,	"Capcom",	"Forgotten Worlds (World, newer)", GAME_SUPPORTS_SAVE )
GAME( 1988,	forgottna,	forgottn,	cps1_10MHz,	forgottn,	forgottn,	ROT0,	"Capcom",	"Forgotten Worlds (World)", GAME_SUPPORTS_SAVE )	// (c) Capcom U.S.A. but World "warning"
GAME( 1988,	forgottnu,	forgottn,	cps1_10MHz,	forgottn,	forgottn,	ROT0,	"Capcom",	"Forgotten Worlds (USA, 88621B-2 B-Board, rev C)", GAME_SUPPORTS_SAVE )
GAME( 1988,	forgottnu1,	forgottn,	cps1_10MHz,	forgottn,	forgottn,	ROT0,	"Capcom",	"Forgotten Worlds (USA, 88618B-2 B-Board, rev C)", GAME_SUPPORTS_SAVE )
GAME( 1988,	forgottnua,	forgottn,	cps1_10MHz,	forgottn,	forgottn,	ROT0,	"Capcom",	"Forgotten Worlds (USA, 88618B-2 B-Board, rev A)", GAME_SUPPORTS_SAVE )
...
GAME( 1991,	knightsh,	knights,	cps1_10MHz,	knightsh,	cps1,		ROT0,	"hack",		"Knights of the Round (hack set 1)", GAME_SUPPORTS_SAVE )
GAME( 1991,	knightsh2,	knights,	cps1_10MHz,	knightsh,	cps1,		ROT0,	"hack",		"Knights of the Round (hack set 2, 911127 etc)", GAME_SUPPORTS_SAVE )
...
GAME( 1990,	mercsj,		mercs,		cps1_10MHz,	mercs,		cps1,		ROT270,	"Capcom",	"Senjou no Ookami II (Ookami 2 900302 Japan)", GAME_SUPPORTS_SAVE )
//
...
GAME( 1995,	pang3j,		pang3,		pang3,		pang3b,		pang3,		ROT0,	"Mitchell",	"Pang! 3: Kaitou Tachi no Karei na Gogo (Japan 950511)", GAME_SUPPORTS_SAVE )
GAME( 1995,	pang3r1a,	pang3,		pang3,		pang3b,		pang3,		ROT0,	"Mitchell",	"Pang! 3 (Euro 950511, alt)", GAME_SUPPORTS_SAVE )
GAME( 1995,	pang3r1,	pang3,		pang3,		pang3,		pang3,		ROT0,	"Mitchell",	"Pang! 3 (Euro 950511)", GAME_SUPPORTS_SAVE )
GAME( 1995,	pang3b,		pang3,		pang3,		pang3b,		pang3b,		ROT0,	"bootleg",	"Pang! 3 (bootleg set 1, Euro 950511)", GAME_SUPPORTS_SAVE )
...
GAME( 1994,	qadjr,		qad,		cps1_12MHz,	qadjr,		cps1,		ROT0,	"Capcom",	"Quiz & Dragons: Capcom Quiz Game (Japan Resale Ver. 940921)", GAME_SUPPORTS_SAVE )
//
...
//
GAME( 1995,	sfzch,		0,		cps1_12MHz,	sfzch,		cps1,		ROT0,	"Capcom",	"Street Fighter Zero (CPS Changer, Japan 951020)", GAME_SUPPORTS_SAVE )
//
...
//
GAME( 1995,	wofch,		0,		qsound,		wofch,		wof,		ROT0,	"Capcom",	"Tenchi wo Kurau II: Sekiheki no Tatakai (CPS Changer, Japan 921031)", GAME_SUPPORTS_SAVE )
GAME( 2010,	wofchdx,	wofch,		qsound,		wofch,		wof,		ROT0,	"hack",		"Sangokushi III Gaiden: Kakou-On's Revenge DX (hack)", GAME_SUPPORTS_SAVE )
//
...
GAME( 1991,	sf2ed,		sf2,		cps1_10MHz,	sf2,		cps1,		ROT0,	"Capcom",	"Street Fighter II: The World Warrior (World 910318)", GAME_SUPPORTS_SAVE )	// "ETC"
GAME( 1991,	sf2ee,		sf2,		cps1_10MHz,	sf2,		sf2ue,		ROT0,	"Capcom",	"Street Fighter II: The World Warrior (World 910228)", GAME_SUPPORTS_SAVE )	// "ETC"
...
GAME( 1992,	sf2amf4,	sf2ce,		cps1_12MHz,	sf2hack,	sf2hack,	ROT0,	"bootleg",	"Street Fighter II': Champion Edition (Alpha Magic-F bootleg set 4, 920313 etc)", GAME_SUPPORTS_SAVE )
GAME( 1992,	sf2amf5,	sf2ce,		cps1_12MHz,	sf2hack,	sf2hack,	ROT0,	"bootleg",	"Street Fighter II': Champion Edition (Alpha Magic-F bootleg set 5, 920313 etc)", GAME_SUPPORTS_SAVE )
...
GAME( 1992,	sf2rb,		sf2ce,		cps1_12MHz,	sf2,		sf2rb,		ROT0,	"bootleg",	"Street Fighter II': Champion Edition (Rainbow bootleg set 1, 920322 etc)", GAME_SUPPORTS_SAVE )   // 920322 - based on World version
GAME( 1992,	sf2rb2,		sf2ce,		cps1_12MHz,	sf2,		sf2rb,		ROT0,	"bootleg",	"Street Fighter II': Champion Edition (Rainbow bootleg set 2, 920322 etc)", GAME_SUPPORTS_SAVE )   // 920322 - based on World version
GAME( 1992,	sf2rb3,		sf2ce,		cps1_12MHz,	sf2,		cps1,		ROT0,	"bootleg",	"Street Fighter II': Champion Edition (Rainbow bootleg set 3, 920322 etc)", GAME_SUPPORTS_SAVE )   // 920322 - based on World version
